...
function SnackbarCPNT(props) {
  ...
  return (
    <>
      <Snackbar
        open={open}
        autoHideDuration={duration ? (duration === "null" ? null : duration) : 5000}
        onClose={handleClose}
        anchorOrigin={anchor ? anchorList[anchor] : anchorList.bc}
        {...rest}
      >
        <Alert
          variant="filled"
        ...
        >
          {children}{" "}
          {loading && <CircularProgress style={{ color: "white", marginLeft: "10px" }} size={15} />}
        </Alert>
      </Snackbar>
    </>
  );
